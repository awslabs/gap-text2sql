...
local config_gen = function(atts) {
    description: "Ablations for Spider 1204 glove, att numbers %s" % [atts],
        ...
        local config_att = config + {att: att};
        {
            ...
            command: [
              ("philly/entry_point_cuda92.sh " +
               "python3 -u train.py " +
               "--config configs/spider-20190205/nl2code-1204-glove-ablations.jsonnet " +
               "--config-args %s " +
...
{
    'spider-1204-ablations-att0.yaml': config_gen([0]),
    'spider-1204-ablations-att1to4.yaml': config_gen([1, 2, 3, 4]),
}
