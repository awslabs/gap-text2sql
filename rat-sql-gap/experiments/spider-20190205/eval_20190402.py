...
def main():
    ...
    for output_from, emb, min_freq in itertools.product(
        ...
        for step in steps:
            logdir = 'logdirs/20190402/output_from=%(output_from)s,emb=%(emb)s,min_freq=%(min_freq)d,att=0' % dict(
            ...
            infer_command = ((
                'python infer.py --config configs/spider-20190205/nl2code-0402.jsonnet '
                + '--config-args "{{output_from: {output_from}, ' +
                'emb: \'{emb}\', min_freq: {min_freq}, att: 0}}" ' + '--logdir logdirs/20190402 ' +
            ...
            eval_command = ((
                'python eval.py --config configs/spider-20190205/nl2code-0402.jsonnet '
                + '--config-args "{{output_from: {output_from}, ' +
