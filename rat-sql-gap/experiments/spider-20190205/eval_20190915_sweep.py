...
CONFIG_PATH = 'configs/spider-20190205/nl2code-0715-align.jsonnet'
...
def create_exp_info(name, commands, data_dir):
    ...
    assert cs[0] == 'philly/spider-0815-search.sh'

    info_dict = {
        'Batch size': int(cs[1]),
        'Learning rate': float(cs[2]),
        'Top K learnable': float(cs[3]),
        'Decoder size': float(cs[4]),
        'Decoder dropout': float(cs[5]),
        'Num layers': float(cs[6]),
    }
    args_str = f"{{bs: {cs[1]}, lr: {cs[2]}, top_k_learnable: {cs[3]}, decoder_recurrent_size: {cs[4]}, decoder_dropout: {cs[5]}, num_layers: {cs[6]}, end_lr: 0, att: 0, setting: 'basic', loss_type: 'softmax', data_path: '{data_dir}/'}}"
