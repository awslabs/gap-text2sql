...
from relogic.pretrainkit.datasets.text_generation.entity_to_text import Entity2QueryDataset, DataCollatorForEntity2Query
...
def get_dataset(args: DataTrainingArguments, tokenizer: PreTrainedTokenizer, evaluate=False):
    ...
    return Entity2QueryDataset(tokenizer=tokenizer, file_path=file_path, block_size=args.block_size)
...
def main():
    ...
    # )
    data_collator = DataCollatorForEntity2Query(tokenizer=tokenizer)

    label_bos_id = data_collator.label_bos_id
    label_eos_id = data_collator.label_eos_id

    match_sequence_scorer = TextGenerationScorer(bos_id=label_bos_id, eos_id=label_eos_id, tokenizer=tokenizer, output_path=os.path.join(training_args.output_dir, data_args.dump_file_name))
