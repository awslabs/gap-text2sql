...
class BARTParser(nn.Module):
  ...
  def forward(self, *input, **kwargs):
    ...
    # copy_span = kwargs.pop("copy_span", None)
    copy_span = None
    ...
    if self.training:
      ...
      outputs = self.bert(input_token_ids, column_spans=column_spans, copy_span=copy_span,
      ...
      # lm_labels = label_ids[:, 1:].clone()
      # lm_labels[label_ids[:, 1:] == label_padding_id] = -100
      # outputs = self.bert(input_token_ids, column_spans=column_spans,
      #                     attention_mask=attention_mask, decoder_input_ids=y_ids, lm_labels=lm_labels, )
