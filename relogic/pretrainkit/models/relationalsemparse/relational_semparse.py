...
from relogic.pretrainkit.models.relationalsemparse.modeling_relational_bart import (PretrainedBartModel, RelationalBartModel,
...
class RelationalBartForTextToSQL(PretrainedBartModel):
    base_model_prefix = "model"

    def __init__(self, config: BartConfig):
        super().__init__(config)
        base_model = RelationalBartModel(config)
    ...
    def prepare_inputs_for_generation(
          self, decoder_input_ids, encoder_past, decoder_past, attention_mask, use_cache, **kwargs):
        assert encoder_past is not None, "past has to be defined for encoder_outputs"

        # first step, decoder_cached_states are empty
        # if not past[1]:
        #     encoder_outputs, decoder_cached_states = encoder_past, None
        # else:
        #     encoder_outputs, decoder_cached_states = encoder_past
        return {
            "input_ids": None,  # encoder_outputs is defined. input_ids not needed
            "example_info_list": [], # Do not need the example info list because all the info the in encoder_outputs
            "encoder_outputs": encoder_past,
            "decoder_cached_states": decoder_past,
            "decoder_input_ids": decoder_input_ids,
            "attention_mask": attention_mask,
    ...
    def generate(
        ...
        assert len(example_info_list) > 0
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs: tuple = encoder(input_ids, attention_mask=attention_mask, example_info_list=example_info_list)
        ...
        if self.config.is_encoder_decoder:
            ...
            # Because the encoder output format ((question, column, table), _, _)
            assert (
                  batch_size == encoder_outputs[0][0][0].shape[0]
            ), f"expected encoder_outputs[0] to have 1st dimension bs={batch_size}, got {encoder_outputs[0][0][0].shape[0]} "
            ...
            # We will use different attention mask for the decoding process than the one used in the encoding
            question_expanded = encoder_outputs[0][0][0].index_select(0, expanded_batch_idxs)
            question_mask_expanded = encoder_outputs[0][0][1].index_select(0, expanded_batch_idxs)
            column_expanded = encoder_outputs[0][1][0].index_select(0, expanded_batch_idxs)
            column_mask_expanded = encoder_outputs[0][1][1].index_select(0, expanded_batch_idxs)
            table_expanded = encoder_outputs[0][2][0].index_select(0, expanded_batch_idxs)
            table_mask_expanded = encoder_outputs[0][2][1].index_select(0, expanded_batch_idxs)
            encoder_outputs = (((question_expanded, question_mask_expanded),
                                (column_expanded, column_mask_expanded),
                                (table_expanded, table_mask_expanded)), *encoder_outputs[1:])
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            model_inputs = self.prepare_inputs_for_generation(
                input_ids, encoder_past=encoder_outputs, decoder_past=past[0], attention_mask=attention_mask, use_cache=use_cache, **model_specific_kwargs
            )
